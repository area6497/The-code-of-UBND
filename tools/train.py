...
def main():
    ...
    spec = importlib.util.spec_from_file_location("mobilenet_v4_", config_path)
    config_module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(config_module)
    kd_cfg = getattr(config_module, "kd_cfg", None)
    ...
    # === 新增：保存 KD 配置 ===
    meta['kd'] = kd_cfg if kd_cfg is not None else {'enable': False}
