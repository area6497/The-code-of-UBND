...
def train(model, runner, lr_update_func, device, epoch, epoches, test_cfg, meta):
    # === KD 知识蒸馏初始化 ===
    kd_cfg = meta.get('kd', None)
    teacher = None

    if kd_cfg and kd_cfg.get('enable', False):
        print(f"✅ 启用知识蒸馏 | Teacher: {kd_cfg['teacher']} | α={kd_cfg['alpha']} | T={kd_cfg['T']}")
        
        teacher_name = kd_cfg['teacher']
        teacher = getattr(models, teacher_name)(pretrained=True)
    
        # === 关键修改：替换分类头，让 teacher 也输出 2 维 ===
        if teacher_name == 'mobilenet_v2':
            in_features = teacher.classifier[1].in_features
            teacher.classifier = torch.nn.Sequential(
                torch.nn.Linear(in_features, 512),
                torch.nn.ReLU(inplace=True),
                torch.nn.Dropout(0.3),
                torch.nn.Linear(512, 2)  # 🔥 2类输出
            )
    
        elif teacher_name == 'mobilenet_v3_large':
            in_features = teacher.classifier[3].in_features
            teacher.classifier[3] = torch.nn.Linear(in_features, 2)
    
        else:
            # 通用 fallback：自动检测最后一层全连接
            for name, module in reversed(list(teacher.named_modules())):
                if isinstance(module, torch.nn.Linear):
                    in_features = module.in_features
                    setattr(teacher, name, torch.nn.Linear(in_features, 2))
                    print(f"⚙️ 自动替换 {teacher_name} 的最后线性层为 2 类输出")
                    break
    
        teacher = teacher.to(device)
        teacher.eval()
    
        if kd_cfg.get('teacher_weights'):
            teacher.load_state_dict(torch.load(kd_cfg['teacher_weights'], map_location=device))
    
        if kd_cfg.get('freeze_teacher', True):
    ...
    with tqdm(total=len(runner.get('train_loader')),desc=f'Train: Epoch {epoch + 1}/{epoches}', postfix=dict, mininterval=0.3) as pbar:
        for iter, batch in enumerate(runner.get('train_loader')):            
            ...
            if kd_cfg and kd_cfg.get('enable', False):
                ...
                student_logits, losses = model(images, targets=targets, return_loss=True, train_statu=True)
            
                ce_loss = losses['loss']  # 分类交叉熵
                kd_loss = F.kl_div(
                    F.log_softmax(student_logits / kd_cfg['T'], dim=1),
                    F.softmax(teacher_logits / kd_cfg['T'], dim=1),
                    reduction='batchmean'
                ) * (kd_cfg['T'] ** 2)
            
                total_loss = (1 - kd_cfg['alpha']) * ce_loss + kd_cfg['alpha'] * kd_loss
                total_loss.backward()
                runner.get('optimizer').step()
            
                losses['loss'] = total_loss  # 替换显示的损失
                preds = student_logits
