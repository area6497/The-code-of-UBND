...
def get_info(classes_path):
    ...
    for data in class_names:
        ...
        if not data.strip() or len(data.split(' ')) < 2:
...
def validation(model, runner, cfg, device, epoch, epoches, meta):
    ...
    with torch.no_grad():
        with tqdm(total=len(runner.get('val_loader')), desc=f'Test : Epoch {epoch + 1}/{epoches}', postfix=dict, mininterval=0.3) as pbar:
            for iter, batch in enumerate(runner.get('val_loader')):
                images, targets, _ = batch
                # ensure tensors on device for model forward
    ...
    # === 新增：准备返回用于绘制 ROC 的真实标签和预测概率 ===
    # 将 pred_list / target_list 拼接并转为 numpy
    try:
        all_preds = torch.cat(pred_list).detach().cpu()
        all_targets = torch.cat(target_list).detach().cpu().numpy()
    except Exception as e:
        # 出错时保持向后兼容：返回空数组
        print(f"[Warning] Concatenating preds/targets failed: {e}")
        all_preds = None
        all_targets = np.array([])

    all_probs = None
    if all_preds is not None:
        # all_preds 的形状可能是 (N, C) 或 (N,) 或 (N,1)
        preds_np = all_preds.numpy()
        if preds_np.ndim == 1:
            # 形如 (N,) -> 二分类或回归得分，视为单 logit，转成 (N,2)
            probs_pos = 1 / (1 + np.exp(-preds_np))  # sigmoid
            probs_pos = probs_pos.reshape(-1, 1)
            all_probs = np.concatenate([1 - probs_pos, probs_pos], axis=1)
        elif preds_np.ndim == 2 and preds_np.shape[1] == 1:
            # (N,1) 单 logit -> sigmoid -> (N,2)
            probs_pos = 1 / (1 + np.exp(-preds_np[:, 0]))
            probs_pos = probs_pos.reshape(-1, 1)
            all_probs = np.concatenate([1 - probs_pos, probs_pos], axis=1)
        elif preds_np.ndim == 2 and preds_np.shape[1] > 1:
            # (N,C) -> 假设为 logits，使用 softmax 获得每类概率
            # 使用 torch.softmax 更稳定，但这里 preds_np 是 numpy，转换回 tensor 方便调用
            with torch.no_grad():
                all_probs = F.softmax(torch.from_numpy(preds_np), dim=1).numpy()
        else:
            # 其它情况，尝试归一化
            try:
                arr = preds_np.reshape(preds_np.shape[0], -1)
                # softmax along last dim
                with torch.no_grad():
                    all_probs = F.softmax(torch.from_numpy(arr), dim=1).numpy()
            except Exception:
                all_probs = None

    if all_probs is None:
        # 仍为 None 时退回到一个占位（避免主进程代码因 None 报错）
        all_probs = np.zeros((all_targets.shape[0], 1))

    # 返回：val_loss, val_acc_num, labels, probs, full_eval_results（便于调试/扩展）
    return val_loss, eval_results.get('accuracy_top-1', 0.0), all_targets, all_probs, eval_results
